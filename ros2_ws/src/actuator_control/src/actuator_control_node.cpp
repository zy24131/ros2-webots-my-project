...
private:
  ...
  {
    out.clear();
    for (const fsm::SteeringJointSpec & joint : fsm::kSteeringJoints) {
  ...
  void ExecuteAction()
  {
    ...
    std::array<double, 8> positions{};
    for (size_t i = 0; i < fsm::kSteeringJoints.size(); ++i) {
      const auto it = cmd.find(fsm::kSteeringJoints[i].name);
      if (it != cmd.end()) {
    ...
    if (max_err <= tolerance) {
      ...
      for (size_t i = 0; i < fsm::kSteeringJoints.size(); ++i) {
        const auto it = command_positions_.find(fsm::kSteeringJoints[i].name);
